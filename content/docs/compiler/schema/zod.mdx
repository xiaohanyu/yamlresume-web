...
| Prop        | Type                                                                                         | Required | Validation Rules                |
| ----------- | -------------------------------------------------------------------------------------------- | -------- | ------------------------------- |
| `layouts[]` | [`DocxLayoutSchema`](#docxlayoutschema) \| [`HtmlLayoutSchema`](#htmllayoutschema) \| [`LatexLayoutSchema`](#latexlayoutschema) \| [`MarkdownLayoutSchema`](#markdownlayoutschema) | ⬜       | Array of layout configurations. |
...
Validates DOCX layout configuration.

| Prop          | Type                                                  | Required | Validation Rules                                      |
| ------------- | ----------------------------------------------------- | -------- | ----------------------------------------------------- |
| `engine`      | `'docx'`                                              | ✅       | Must be "docx"                                        |
| `advanced?`   | [`DocxAdvancedSchema`](#docxadvancedschema)           | ⬜       | See [advanced schema](#docxadvancedschema)            |
| `page?`       | [`DocxPageSchema`](#docxpageschema)                   | ⬜       | See [page schema](#docxpageschema)                    |
| `sections?`   | [`SectionsSchema`](#sectionsschema)                   | ⬜       | See [sections schema](#sectionsschema)                |
| `template?`   | [`DocxTemplate`](#docxtemplate)                       | ⬜       | Must be from predefined [template options](#docxtemplate) |
| `typography?` | [`DocxTypographySchema`](#docxtypographyschema)       | ⬜       | See [typography schema](#docxtypographyschema)        |

##### DocxAdvancedSchema

Validates advanced DOCX configuration options.
...
| ------------------ | ---------------------- | -------- | ---------------------------------------------- |
| `margins?`         | [`MarginsSchema`](#marginsschema) | ⬜       | See [margins schema](#marginsschema)           |
| `paperSize?`       | [`DocxPaperSize`](#docxpapersize) | ⬜       | Must be from predefined [paper size options](#docxpapersize) |
| `showPageNumbers?` | `boolean`              | ⬜       | true/false                                     |
...
| Prop          | Type                              | Required | Validation Rules                                           |
| ------------- | --------------------------------- | -------- | ---------------------------------------------------------- |
| `fontSize?`   | [`DocxFontSize`](#docxfontsize)   | ⬜       | Must be from predefined [font size options](#docxfontsize) |
| `fontFamily?` | `string`                          | ⬜       | A comma-separated list of font families.                   |
| `lineSpacing?`| [`LineSpacing`](#linespacing)     | ⬜       | Must be from predefined [line spacing options](#linespacing) |
...
Validates font family names for DOCX output.

**Validation Rules:**

- Must be a valid font family name available on the target system
...
A union type for all possible DOCX font size options.

**Validation Rules:**

- Must be one of: `10pt`, `10.5pt`, `11pt`, `11.5pt`, `12pt`
...
- Must be one of: `calm`
- See also: [templates](/docs/layouts/docx/templates)
