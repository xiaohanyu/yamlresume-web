...
| 属性        | タイプ                                                                                         | 必須 | 検証ルール            |
| ----------- | -------------------------------------------------------------------------------------------- | -------- | ------------------------------- |
| `layouts[]` | [`DocxLayoutSchema`](#docxlayoutschema) \| [`HtmlLayoutSchema`](#htmllayoutschema) \| [`LatexLayoutSchema`](#latexlayoutschema) \| [`MarkdownLayoutSchema`](#markdownlayoutschema) | ⬜       | レイアウト設定の配列。 |
...
| `lineSpacing?` | [`LineSpacing`](#linespacing)      | ⬜       | 事前定義された[行間オプション](#linespacing) からである必要があります |
...
| 属性          | タイプ                                                  | 必須 | 検証ルール                                      |
| ------------- | ----------------------------------------------------- | -------- | ----------------------------------------------------- |
| `engine`      | `'docx'`                                              | ✅       | "docx" でなければなりません                                        |
| `advanced?`   | [`DocxAdvancedSchema`](#docxadvancedschema)           | ⬜       | [高度なスキーマ](#docxadvancedschema) を参照                |
| `page?`       | [`DocxPageSchema`](#docxpageschema)                   | ⬜       | [ページスキーマ](#docxpageschema) を参照                   |
| `sections?`   | [`SectionsSchema`](#sectionsschema)                   | ⬜       | [セクションスキーマ](#sectionsschema) を参照                |
| `template?`   | [`DocxTemplate`](#docxtemplate)                       | ⬜       | 事前定義された[テンプレートオプション](#docxtemplate) からである必要があります |
| `typography?` | [`DocxTypographySchema`](#docxtypographyschema)       | ⬜       | [タイポグラフィスキーマ](#docxtypographyschema) を参照      |

##### DocxAdvancedSchema

DOCX の高度な設定オプションを検証します。
...
| ------------------ | ---------------------- | -------- | ------------------------------------ |
| `margins?`         | [`MarginsSchema`](#marginsschema) | ⬜       | [マージンスキーマ](#marginsschema) を参照 |
| `paperSize?`       | [`DocxPaperSize`](#docxpapersize) | ⬜       | 事前定義された[用紙サイズオプション](#docxpapersize) からである必要があります |
| `showPageNumbers?` | `boolean`              | ⬜       | true/false                           |
...
| 属性          | タイプ                              | 必須 | 検証ルール                                            |
| ------------- | --------------------------------- | -------- | ----------------------------------------------------- |
| `fontSize?`   | [`DocxFontSize`](#docxfontsize)   | ⬜       | 事前定義された[フォントサイズオプション](#docxfontsize) からである必要があります |
| `fontFamily?` | `string`                          | ⬜       | フォントファミリーのカンマ区切りリスト。                   |
| `lineSpacing?`| [`LineSpacing`](#linespacing)     | ⬜       | 事前定義された[行間オプション](#linespacing) からである必要があります |
...
DOCX 出力用のフォントファミリー名を検証します。

**検証ルール：**

- ターゲットシステムで利用可能な有効なフォントファミリー名である必要があります
- 例：`Arial`、`Times New Roman`、`Calibri`
...
すべての可能な DOCX フォントサイズオプションのユニオンタイプ。

**検証ルール：**

- 以下のいずれかである必要があります：`10pt`、`10.5pt`、`11pt`、`11.5pt`、`12pt`
- `10pt` がデフォルトフォントサイズです

### DocxPaperSize

すべての可能な DOCX 用紙サイズオプションのユニオンタイプ。
...
すべての可能な DOCX テンプレートオプションのユニオンタイプ。

**検証ルール：**

- 以下のいずれかである必要があります：`calm`
- 参照：[テンプレート](/ja/docs/layouts/docx/templates)
