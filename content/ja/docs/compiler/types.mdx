...
| 属性        | タイプ                                                                 | 必須 | 説明                            |
| ----------- | -------------------------------------------------------------------- | -------- | -------------------------------------- |
| `layouts[]` | [`DocxLayout`](#docxlayout) \| [`HtmlLayout`](#htmllayout) \| [`LatexLayout`](#latexlayout) \| [`MarkdownLayout`](#markdownlayout) | ⬜       | 複数の出力レイアウト設定。 |
...
| `lineSpacing?` | [`LineSpacing`](#linespacing)       | ⬜       | 文書の行間。          |
...
| `showIcons?` | `boolean`                            | ⬜       | リンクとプロフィールのアイコンを表示するかどうか。 |
| `showUrls?`  | `boolean`                            | ⬜       | 生の URL を表示するか、適切なテキストの背後に隠すか。 |
...
| 属性          | タイプ                                  | 必須 | 説明                                           |
| ------------- | ------------------------------------- | -------- | ----------------------------------------------------- |
| `engine`      | `'docx'`                              | ✅       | 履歴書を生成するエンジン。                    |
| `advanced?`   | [`DocxAdvanced`](#docxadvanced)       | ⬜       | 高度な設定オプションを定義します。               |
| `page?`       | [`DocxPage`](#docxpage)               | ⬜       | 文書プレゼンテーションのページレベル設定を定義します。|
| `sections?`   | [`Sections`](#sections)               | ⬜       | セクションのカスタマイズ設定を定義します。               |
| `template?`   | [`DocxTemplate`](#docxtemplate)       | ⬜       | 選択されたテンプレートを定義します。                        |
| `typography?` | [`DocxTypography`](#docxtypography)   | ⬜       | 文書フォーマットのタイポグラフィ設定を定義します。  |

##### DocxAdvanced

DOCX の高度な設定オプションを定義します。
...
DOCX の文書プレゼンテーションのページレベル設定を定義します。

| 属性               | タイプ                   | 必須 | 説明                      |
| ------------------ | ---------------------- | -------- | -------------------------------- |
| `margins?`         | [`Margins`](#margins)  | ⬜       | ページマージン設定を定義します。    |
| `paperSize?`       | [`DocxPaperSize`](#docxpapersize) | ⬜       | 用紙サイズを定義します。 |
| `showPageNumbers?` | `boolean`              | ⬜       | ページ番号を表示するかどうか。 |

##### DocxTypography

DOCX の文書フォーマットのタイポグラフィ設定を定義します。

| 属性          | タイプ                                | 必須 | 説明                        |
| ------------- | ----------------------------------- | -------- | -------------------------------- |
| `fontSize?`   | [`DocxFontSize`](#docxfontsize)     | ⬜       | 文書の基本フォントサイズ。 |
| `fontFamily?` | `string`                            | ⬜       | 使用するフォントファミリーのカンマ区切りリスト。 |
| `lineSpacing?`| [`LineSpacing`](#linespacing)       | ⬜       | 文書の行間。           |
...
- `10.5pt` - 10.5pt フォントサイズ
- `11pt` - 11pt フォントサイズ
- `11.5pt` - 11.5pt フォントサイズ
- `12pt` - 12pt フォントサイズ
...
これらのセマンティック値は Tailwind CSS のレディングスケールに従います：
