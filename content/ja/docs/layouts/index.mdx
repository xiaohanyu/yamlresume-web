---
title: レイアウト
---

履歴書の核心はコンテンツですが、レイアウトも重要です。多くの JSON/YAML ベースの履歴書ジェネレーターはコンテンツのみに焦点を当て、レイアウトを変更せずに残し、ユーザーに大きな問題を残しています。

他の履歴書ジェネレーターとは異なり、YAMLResume はすぐに使えるレイアウトソリューションを提供し、ユーザーが履歴書のコンテンツに集中しながら、ピクセルパーフェクトなレイアウトとタイポグラフィでプロフェッショナルな履歴書を生成できるようにします。

v0.8.0 以降、YAMLResume は `layouts` 配列を通じて、複数のレイアウトと複数の出力エンジンをサポートしています。

以下は `resume.yml` ファイルでのレイアウト設定の例です：

```yml lineNumbers title="resume.yml"
---
content:
  # ...

locale:
  language: en

layouts:
  - engine: latex
    template: moderncv-banking
    page:
      showPageNumbers: true
      margins:
        top: 2.5cm
        left: 1.5cm
        right: 1.5cm
        bottom: 2.5cm
    typography:
      fontSize: 11pt
  - engine: markdown
```

## エンジン

YAMLResume は現在、4 つのエンジンをサポートしています。

- [LaTeX](./layouts/latex): 高品質な PDF 履歴書を生成します。
- [Markdown](./layouts/markdown): プレーンな Markdown 履歴書を生成します。
- [HTML](./layouts/html): クリーンでウェブ対応の HTML 履歴書を生成します。
- [DOCX](./layouts/docx): Microsoft Word ドキュメントを生成します。

## 詳細情報

<Cards>
  <Card
    href="./layouts/sections"
    title="セクション"
    description="履歴書のセクションをカスタマイズ"
  />
  <Card
    href="./layouts/latex"
    title="LaTeX エンジン"
    description="LaTeX で PDF 履歴書を生成"
  />
  <Card
    href="./layouts/markdown"
    title="Markdown エンジン"
    description="Markdown 履歴書を生成"
  />
  <Card
    href="./layouts/html"
    title="HTML エンジン"
    description="HTML 履歴書を生成"
  />
  <Card
    href="./layouts/docx"
    title="DOCX エンジン"
    description="Microsoft Word ドキュメントを生成"
  />
</Cards>
