...
| 屬性        | 類型                                                                                         | 必需 | 驗證規則            |
| ----------- | -------------------------------------------------------------------------------------------- | -------- | ------------------------------- |
| `layouts[]` | [`DocxLayoutSchema`](#docxlayoutschema) \| [`HtmlLayoutSchema`](#htmllayoutschema) \| [`LatexLayoutSchema`](#latexlayoutschema) \| [`MarkdownLayoutSchema`](#markdownlayoutschema) | ⬜       | 佈局配置陣列。 |
...
| `lineSpacing?` | [`LineSpacing`](#linespacing)      | ⬜       | 必須來自預定義的[行距選項](#linespacing) |
...
| 屬性          | 類型                                                  | 必需 | 驗證規則                                      |
| ------------- | ----------------------------------------------------- | -------- | ----------------------------------------------------- |
| `engine`      | `'docx'`                                              | ✅       | 必須是 "docx"                                        |
| `advanced?`   | [`DocxAdvancedSchema`](#docxadvancedschema)           | ⬜       | 參見[進階模式](#docxadvancedschema)            |
| `page?`       | [`DocxPageSchema`](#docxpageschema)                   | ⬜       | 參見[頁面模式](#docxpageschema)                    |
| `sections?`   | [`SectionsSchema`](#sectionsschema)                   | ⬜       | 參見[分節模式](#sectionsschema)                |
| `template?`   | [`DocxTemplate`](#docxtemplate)                       | ⬜       | 必須來自預定義的[模板選項](#docxtemplate) |
| `typography?` | [`DocxTypographySchema`](#docxtypographyschema)       | ⬜       | 參見[排版模式](#docxtypographyschema)        |
...
| ------------------ | ---------------------- | -------- | ---------------------------------------------- |
| `margins?`         | [`MarginsSchema`](#marginsschema) | ⬜       | 參見[頁邊距模式](#marginsschema)           |
| `paperSize?`       | [`DocxPaperSize`](#docxpapersize) | ⬜       | 必須來自預定義的[紙張大小選項](#docxpapersize) |
| `showPageNumbers?` | `boolean`              | ⬜       | true/false                                     |
...
| 屬性          | 類型                              | 必需 | 驗證規則                                           |
| ------------- | --------------------------------- | -------- | ---------------------------------------------------------- |
| `fontSize?`   | [`DocxFontSize`](#docxfontsize)   | ⬜       | 必須來自預定義的[字體大小選項](#docxfontsize) |
| `fontFamily?` | `string`                          | ⬜       | 逗號分隔的字體家族列表。                   |
| `lineSpacing?`| [`LineSpacing`](#linespacing)     | ⬜       | 必須來自預定義的[行距選項](#linespacing) |
...
所有可能 DOCX 字體大小選項的聯合類型。

**驗證規則：**

- 必須是以下之一：`10pt`、`10.5pt`、`11pt`、`11.5pt`、`12pt`
- `10pt` 是預設字體大小

### DocxPaperSize

所有可能 DOCX 紙張大小選項的聯合類型。
...
- 必須是以下之一：`calm`
- 另請參閱：[模板](/zh-tw/docs/layouts/docx/templates)
