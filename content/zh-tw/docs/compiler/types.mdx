...
| 屬性        | 類型                                                                 | 必需 | 描述                            |
| ----------- | -------------------------------------------------------------------- | -------- | -------------------------------------- |
| `layouts[]` | [`DocxLayout`](#docxlayout) \| [`HtmlLayout`](#htmllayout) \| [`LatexLayout`](#latexlayout) \| [`MarkdownLayout`](#markdownlayout) | ⬜       | 多個輸出佈局配置。 |
...
| `lineSpacing?` | [`LineSpacing`](#linespacing)      | ⬜       | 文件的行距。          |
...
| `showUrls?`  | `boolean`                            | ⬜       | 是否顯示原始 URL 或將其隱藏在合理的文字後面。 |
...
| ------------- | ------------------------------------- | -------- | ----------------------------------------------------- |
| `engine`      | `'docx'`                              | ✅       | 生成履歷的引擎。                    |
| `advanced?`   | [`DocxAdvanced`](#docxadvanced)       | ⬜       | 定義進階配置選項。               |
| `page?`       | [`DocxPage`](#docxpage)               | ⬜       | 定義文件演示的頁面級設置。|
| `sections?`   | [`Sections`](#sections)               | ⬜       | 定義分節自定義設置。               |
| `template?`   | [`DocxTemplate`](#docxtemplate)       | ⬜       | 定義選定的模板。                        |
| `typography?` | [`DocxTypography`](#docxtypography)   | ⬜       | 定義文件格式的排版設置。  |
...
| ------------------ | ---------------------- | -------- | -------------------------------- |
| `margins?`         | [`Margins`](#margins)  | ⬜       | 定義頁邊距設置。    |
| `paperSize?`       | [`DocxPaperSize`](#docxpapersize) | ⬜       | 定義紙張大小。 |
| `showPageNumbers?` | `boolean`              | ⬜       | 是否顯示頁碼。 |
...
| 屬性          | 類型                                | 必需 | 描述                        |
| ------------- | ----------------------------------- | -------- | -------------------------------- |
| `fontSize?`   | [`DocxFontSize`](#docxfontsize)     | ⬜       | 文件的基礎字體大小。 |
| `fontFamily?` | `string`                            | ⬜       | 要使用的逗號分隔的字體家族列表。 |
| `lineSpacing?`| [`LineSpacing`](#linespacing)       | ⬜       | 文件的行距。        |
...
這些語義值遵循 Tailwind CSS 的 leading 刻度：
