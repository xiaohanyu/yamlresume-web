...
| Prop        | Type                                                                                         | Requis | Règles de Validation            |
| ----------- | -------------------------------------------------------------------------------------------- | ------ | ------------------------------- |
| `layouts[]` | [`DocxLayoutSchema`](#docxlayoutschema) \| [`HtmlLayoutSchema`](#htmllayoutschema) \| [`LatexLayoutSchema`](#latexlayoutschema) \| [`MarkdownLayoutSchema`](#markdownlayoutschema) | ⬜     | Tableau de configurations de mise en page. |
...
| `lineSpacing?` | [`LineSpacing`](#linespacing)     | ⬜     | Doit être parmi les [options d'interligne](#linespacing) prédéfinies |
...
Valide la configuration de mise en page DOCX.

| Prop          | Type                                                  | Requis | Règles de Validation                                      |
| ------------- | ----------------------------------------------------- | ------ | --------------------------------------------------------- |
| `engine`      | `'docx'`                                              | ✅     | Doit être "docx"                                        |
| `advanced?`   | [`DocxAdvancedSchema`](#docxadvancedschema)           | ⬜     | Voir [schéma avancé](#docxadvancedschema)               |
| `page?`       | [`DocxPageSchema`](#docxpageschema)                   | ⬜     | Voir [schéma de page](#docxpageschema)                  |
| `sections?`   | [`SectionsSchema`](#sectionsschema)                   | ⬜     | Voir [schéma de sections](#sectionsschema)              |
| `template?`   | [`DocxTemplate`](#docxtemplate)                       | ⬜     | Doit être parmi les [options de modèle](#docxtemplate) prédéfinies |
| `typography?` | [`DocxTypographySchema`](#docxtypographyschema)       | ⬜     | Voir [schéma de typographie](#docxtypographyschema)     |

##### DocxAdvancedSchema

Valide les options de configuration avancées pour DOCX.
...
| ------------------ | ---------------------- | ------ | -------------------------------------------------- |
| `margins?`         | [`MarginsSchema`](#marginsschema) | ⬜     | Voir [schéma de marges](#marginsschema)            |
| `paperSize?`       | [`DocxPaperSize`](#docxpapersize) | ⬜     | Doit être parmi les [options de taille de papier](#docxpapersize) prédéfinies |
| `showPageNumbers?` | `boolean`              | ⬜     | true/false                                         |
...
| Prop          | Type                              | Requis | Règles de Validation                                           |
| ------------- | --------------------------------- | ------ | ---------------------------------------------------------- |
| `fontSize?`   | [`DocxFontSize`](#docxfontsize)   | ⬜     | Doit être parmi les [options de taille de police](#docxfontsize) prédéfinies |
| `fontFamily?` | `string`                          | ⬜     | Une liste de familles de polices séparées par des virgules. |
| `lineSpacing?`| [`LineSpacing`](#linespacing)     | ⬜     | Doit être parmi les [options d'interligne](#linespacing) prédéfinies |
...
### Schéma Famille de Police DOCX

Valide les noms de familles de polices pour la sortie DOCX.

**Règles de Validation :**

- Doit être un nom de famille de police valide disponible sur le système cible
- Exemples : `Arial`, `Times New Roman`, `Calibri`
...
Un type union pour toutes les options de taille de police DOCX possibles.

**Règles de Validation :**

- Doit être l'une de : `10pt`, `10.5pt`, `11pt`, `11.5pt`, `12pt`
- `10pt` est la taille de police par défaut

### DocxPaperSize

Un type union pour toutes les options de taille de papier DOCX possibles.

**Règles de Validation :**

- Doit être l'une de : `a4`, `letter`
- `a4` est la taille de papier par défaut

### DocxTemplate

Un type union pour toutes les options de modèle DOCX possibles.

**Règles de Validation :**

- Doit être l'une de : `calm`
- Voir aussi : [modèles](/fr/docs/layouts/docx/templates)
...
Un type union pour toutes les options d'interligne possibles.
